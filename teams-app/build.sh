...
MISSING=0
for f in manifest.json ai-plugin.yaml color.png outline.png; do
...
if command -v zip &> /dev/null; then
  zip -j "$OUTPUT" manifest.json ai-plugin.yaml color.png outline.png
...
with zipfile.ZipFile(out, 'w', zipfile.ZIP_DEFLATED) as z:
    for f in ['manifest.json', 'ai-plugin.yaml', 'color.png', 'outline.png']:
